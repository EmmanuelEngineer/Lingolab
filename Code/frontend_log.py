...
class LingoLab(ctk.CTk):
    ...
    def pre_submit(self):
        ...
        self.submit_answer(to_log=True)
    ...
    def log_statistics(self, fluency, accuracy, sentiment, gaze):
        """
        Log statistics for the current exercise, including fluency, accuracy, sentiment, and gaze data.
        """
    ...
    def transcribe_audio(self, file_path):
        ...
        log_entry = (
            ...
            f"  Fluency Score: {fluency_score:.2f}\n"
            f"  Accuracy: {'Correct' if accuracy else 'Incorrect'}\n"
            f"  Sentiment: {sentiment}\n"
    ...
    def submit_answer(self, event=None,to_log=False):
            ...
            if accuracy:
                ...
                # if incorrect check which type of exercise is that
            ...
            # Log statistics
            if to_log:
                self.log_statistics(fluency_score, accuracy, sentiment, gaze_score)

            # remove the input field + button to continue from the exercise page
            ...
            print("Gaze accuracy: ", val, " - Seconds taken: ", diff)
            if self.emotion_recognizer_pause or (diff > 10 and val < self.gaze_distance_treshold):
                ...
                self.end_pause = ctk.CTkButton(self.pause_frame, text="End break",
                                            command=lambda: (self.pause_frame.destroy(), self.next_exercise()))
    ...
    def show_graphs(self):
        ...
        # Accuracy Graph
        plt.subplot(2, 2, 2)
        plt.bar(range(len(self.accuracy_scores)), self.accuracy_scores, color='green')
        plt.title("Accuracy")
        plt.xlabel("Exercise")
        ...
        # Sentiment Graph
        plt.subplot(2, 2, 3)
        sentiments = [s if s != "N/A" else "neutral" for s in self.sentiment_scores]
        sentiment_labels, sentiment_counts = zip(*{s: sentiments.count(s) for s in set(sentiments)}.items())
        plt.bar(sentiment_labels, sentiment_counts, color='orange')
        ...
        # Gaze Graph
        plt.subplot(2, 2, 4)
        plt.plot(self.gaze_scores, marker='x', color='purple')
        plt.title("Gaze Scores")
        plt.xlabel("Exercise")
        plt.ylabel("Score")
        plt.grid()
    ...
    def end_exercises(self):
        # Distruggi il frame degli esercizi
        self.exercise_frame.destroy()

        # Mostra i grafici alla fine degli esercizi
        self.show_graphs()
